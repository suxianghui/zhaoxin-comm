...
import builder from "./components/api/api-common";
import qs from 'qs';
  ...
  send(data) {
    ...
    if (!simulation) {
      let config = {
        ...
        url: me._url,
        method: me._method,
        // headers: { token: localStorage.getItem('pagetoken') },
      ...
      if(option.type === 'BASEURL_03'){
        config.baseURL = window.BASEURL_03
      }
      ...
      // console.log(config,'12341234')
      // if(option.method == 'POST' && (option.baseUrl === builder.BASEURL_02)){
      //   console.log(data,'12341234')
      //   data.secret = "bLo9^Bz#$xHxR";//&secret=bLo9^Bz#$xHxR
      ...
      // if(option.method == 'POST' && (option.baseUrl === builder.BASEURL_05)){
      //   const keys = Object.keys(data).sort();
      //   let _params = '';
      //   keys.map((t, i) => {
      //     if (i < 1)
      ...
      //       _params += '&' + [t, '=', data[t]].join('')
      //   });
      //   let sign =  `${_params}bLo9^Bz#$xHxR`;
      //   if(option.url == "/api/complaint/progressRecord" || option.url == "/api/complaint/businessComplaintList"){
      //     sign =  `${_params}UYHjLD*u1J8X0CguG&WYp9c5FI79kiaw`;
      //   }
      //   console.log(sign,'_params')
      //   data.sign = md5(sign).toUpperCase()
      //   // delete data.secret;
      // }
